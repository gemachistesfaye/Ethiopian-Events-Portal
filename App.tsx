...
const App: React.FC = () => {
  ...
  const handleAddReminder = (dateStr: string, note: string, priority: ReminderPriority, category: ReminderCategory, eventId?: string) => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 selection:bg-amber-200 flex flex-col pb-20 md:pb-0 font-sans">
      ...
      {/* REFINED FOOTER - BRANDED GEMACHIS | DATA & DEV */}
      <footer className="bg-white border-t border-stone-200 mt-20 pt-24 pb-12 px-8">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-16 items-start mb-20">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 mb-2">
                <span className="bg-stone-900 text-amber-500 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-[0.4em]">Developer Profile</span>
              </div>
              <h3 className="text-6xl font-black text-stone-900 leading-tight tracking-tighter">
                Gemachis <br />
                <span className="text-amber-600">| Data & Dev</span>
              </h3>
              <p className="text-stone-500 text-lg font-medium leading-relaxed max-w-md">
                Crafting intelligent solutions at the intersection of cultural heritage and advanced software development.
              </p>
            </div>

            {/* CONTACT & CTA SECTION */}
            <div className="flex flex-col md:items-end">
              <div className="bg-stone-50 p-10 rounded-[3rem] border border-stone-200 shadow-sm w-full max-w-md">
                <p className="text-stone-400 text-[10px] font-black uppercase tracking-[0.3em] mb-4">Lead Architect</p>
                <h4 className="text-2xl font-black text-stone-900 mb-1">Gemachis Tesfaye</h4>
                <p className="text-amber-600 text-xs font-bold uppercase tracking-widest mb-8">Lead Software Engineer</p>
                
                <p className="text-stone-600 text-sm font-bold mb-6">Contact him for collaborations:</p>
                
                <div className="grid grid-cols-4 gap-4">
                  {/* Telegram - Primary Requested */}
                  <a href="https://t.me/your_telegram" target="_blank" rel="noopener noreferrer" className="w-full aspect-square bg-sky-500 text-white rounded-2xl flex items-center justify-center transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-sky-200 shadow-md group" title="Telegram Profile">
                    <svg className="w-7 h-7 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3.002-2.21-1.444 1.394c-.16.16-.294.294-.602.294l.215-3.048 5.549-5.012c.241-.213-.053-.331-.373-.118l-6.853 4.314-2.956-.922c-.642-.201-.655-.642.134-.95l11.554-4.451c.535-.196.994.122.832.797z"/></svg>
                  </a>
                  {/* GitHub */}
                  <a href="https://github.com/your_github" target="_blank" rel="noopener noreferrer" className="w-full aspect-square bg-stone-900 text-white rounded-2xl flex items-center justify-center transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-stone-200 shadow-md group" title="GitHub profile">
                    <svg className="w-7 h-7 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>
                  </a>
                  {/* Email */}
                  <a href="mailto:your_email@domain.com" className="w-full aspect-square bg-rose-500 text-white rounded-2xl flex items-center justify-center transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-rose-200 shadow-md group" title="Email me">
                    <svg className="w-7 h-7 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  </a>
                  {/* Phone */}
                  <a href="tel:+251900000000" className="w-full aspect-square bg-emerald-500 text-white rounded-2xl flex items-center justify-center transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-emerald-200 shadow-md group" title="Call me">
          ...
          <div className="pt-12 border-t border-stone-100 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 bg-stone-900 rounded-lg flex items-center justify-center text-amber-500 font-black text-sm">G</div>
              <p className="text-stone-400 text-xs font-bold tracking-tight">Developed by Gemachis | Data & Dev</p>
            </div>
            <p className="text-stone-300 text-[10px] font-black uppercase tracking-[0.4em]">All Rights Reserved © 2026</p>
          </div>
        </div>
      </footer>
      ...
      {selectedEvent && (
        <EventDetails 
          ...
          onAddReminder={(eventId, note, priority, category) => {
