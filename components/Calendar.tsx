    ...
    return (
      <div className="flex flex-col items-start">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <span className="text-stone-300 text-[9px] font-black uppercase tracking-widest hidden sm:inline-block">
            ({ethRangeEnglish})
          </span>
        </div>
      </div>
              ...
              >
                ...
                {day.isToday && (
                  <div className="absolute top-2 right-2 flex flex-col items-end">
                    <div className="w-2.5 h-2.5 rounded-full bg-amber-900/200 animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.6)]" />
                  </div>
                )}
                
                {/* Gregorian Day (Primary) */}
                <span className={`text-xl font-black leading-none mb-1 ${isSelected ? 'text-amber-500' : 'text-stone-100'}`}>
                ...
                {hasEvent && (
                  <div className="mt-2 flex gap-1">
                    {day.events.map(e => (
                      <div key={e.id} className="w-1.5 h-1.5 rounded-full bg-amber-900/200 shadow-[0_0_5px_rgba(245,158,11,0.5)]" />
                    ))}
                  </div>
                )}
                
                {hasEvent && (
                   <div className={`mt-1 text-[7px] font-black truncate px-2 max-w-full uppercase tracking-tighter ${isSelected ? 'text-amber-400' : 'text-stone-400'}`}>
