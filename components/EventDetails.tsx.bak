      ...
      <div className="bg-stone-50 w-full max-w-3xl rounded-[3rem] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-500 flex flex-col md:flex-row max-h-[90vh]">
        ...
        <div className="md:w-1/3 relative h-64 md:h-auto overflow-hidden">
          ...
          <div className="absolute bottom-6 left-6 text-white z-10">
             <span className="bg-amber-900/200 text-stone-100 px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest mb-3 inline-block shadow-lg">
               {event.category}
             </span>
        ...
        <div className="md:w-2/3 p-10 overflow-y-auto bg-stone-100/30">
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-stone-100 font-bold">{event.ethDate} <span className="mx-2 text-stone-300">|</span> {formattedGregDate}</p>
            </div>
            <button 
              onClick={onClose}
              className="p-3 bg-stone-800 hover:bg-stone-800 rounded-2xl text-stone-400 transition-all shadow-sm"
            >
          ...
          <div className="space-y-10">
            ...
            <section className="bg-amber-900/200 rounded-[2rem] p-8 text-stone-100 shadow-xl shadow-amber-200/50">
              ...
              {showReminderForm ? (
                <form onSubmit={handleSubmitReminder} className="bg-stone-50 p-8 rounded-[2.5rem] border border-stone-200 shadow-2xl animate-in slide-in-from-bottom-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button type="button" onClick={() => setShowReminderForm(false)} className="px-8 py-4 bg-stone-800 text-stone-400 rounded-2xl font-black text-xs uppercase tracking-widest">Cancel</button>
                  </div>
                </form>
              ) : (
                <button 
