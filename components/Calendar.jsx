...
import { generateGregorianCalendar } from '../utils/dateConverter';
...
const Calendar = ({ onSelectDate, selectedDate }) => {
    ...
    return (<div className="bg-stone-50 rounded-3xl shadow-xl shadow-stone-200/50 overflow-hidden border border-stone-200 animate-in fade-in duration-500">
      <div className="bg-stone-50 px-6 py-6 text-stone-900 flex flex-row justify-between items-center border-b border-stone-200">
        <div className="flex-1">
          {headerContent}
        </div>
        <div className="flex gap-2">
          <button onClick={prev} className="p-3 hover:bg-stone-100 rounded-xl border border-stone-200 text-stone-500 transition-all hover:text-amber-600">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M15 19l-7-7 7-7"/></svg>
          </button>
          <button onClick={next} className="p-3 hover:bg-stone-100 rounded-xl border border-stone-200 text-stone-500 transition-all hover:text-amber-600">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {Array.from({ length: firstDayOffset }).map((_, i) => (<div key={`empty-${i}`} className="h-24 bg-stone-100/50 rounded-2xl border border-dashed border-stone-100"/>))}
          
          {days.map((day) => {
            const hasEvent = day.events.length > 0;
            const isSelected = selectedDate?.toDateString() === day.gregorian.toDateString();
            return (<button key={day.gregorian.toISOString()} onClick={() => onSelectDate(day)} className={`h-24 rounded-2xl border flex flex-col items-center justify-center relative transition-all group overflow-hidden
                  ${isSelected ? 'bg-stone-50 border-stone-200 text-stone-900 shadow-2xl scale-[1.05] z-10' : 'bg-stone-50 border-stone-100 hover:border-amber-400 hover:bg-stone-100/50'}
                  ${day.isToday ? 'ring-2 ring-amber-500 ring-offset-2' : ''}
                `}>
                {/* Visual marker for Today */}
                {day.isToday && (<div className="absolute top-2 right-2 flex flex-col items-end">
                    <div className="w-2.5 h-2.5 rounded-full bg-amber-900/20 animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.6)]"/>
                  </div>)}
                ...
                {hasEvent && (<div className="mt-2 flex gap-1">
                    {day.events.map(e => (<div key={e.id} className="w-1.5 h-1.5 rounded-full bg-amber-900/20 shadow-[0_0_5px_rgba(245,158,11,0.5)]"/>))}
                  </div>)}
                
                {hasEvent && (<div className={`mt-1 text-[7px] font-bold truncate px-2 max-w-full uppercase tracking-tighter ${isSelected ? 'text-amber-400' : 'text-stone-500'}`}>
