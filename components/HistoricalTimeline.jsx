...
const CATEGORIES = ['All', 'Politics', 'Culture', 'Religion', 'Warfare', 'Innovation'];
...
const HistoricalTimeline = () => {
    ...
    return (<div className="min-h-screen bg-stone-100 p-6 lg:p-10 font-sans relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-2.5 mb-14 relative z-20">
          {CATEGORIES.map(c => (<button key={c} onClick={() => setFilter(c)} className={`relative overflow-hidden px-5 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all duration-300 group shadow-sm ${filter === c
        ...
        <div className="relative pb-20">
          ...
          <div className="absolute left-6 md:left-1/2 transform md:-translate-x-1/2 w-1 h-full bg-stone-100 rounded-full overflow-hidden shadow-inner">
             <motion.div className="w-full h-1/3 bg-gradient-to-b from-transparent via-amber-400 to-transparent blur-[2px]" animate={{ y: ["-100%", "300%"] }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }}></motion.div>
          </div>

          <div className="space-y-12 md:space-y-20">
            {filteredEvents.map((event, index) => (<motion.div key={event.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: index % 2 === 0 ? 0 : 0.2 }} className={`relative flex flex-col md:flex-row items-center justify-between w-full ${index % 2 === 0 ? 'md:flex-row-reverse' : ''} gap-8 md:gap-0`}>
                {/* Content Card (Gaming Style - Light) */}
                <div className="w-full md:w-[42%] ml-16 md:ml-0 group perspective-1000">
                  <div className="relative bg-stone-50/90 backdrop-blur-xl p-6 md:p-8 rounded-2xl border border-stone-200 shadow-xl transition-all duration-500 cursor-pointer overflow-hidden transform-gpu group-hover:scale-[1.02] group-hover:border-amber-400 group-hover:shadow-[0_15px_40px_rgba(245,158,11,0.15)]" onClick={() => setSelectedEvent(event)}>
                    {/* Corner accents */}
                ...
                <div className="absolute left-6 md:left-1/2 transform -translate-x-1/2 w-8 h-8 rounded-full border-[3px] border-white bg-stone-100 flex items-center justify-center shadow-md z-10 group cursor-pointer" onClick={() => setSelectedEvent(event)}>
                   <div className="w-3 h-3 bg-stone-300 rounded-full transition-all duration-300 group-hover:bg-amber-900/20 group-hover:shadow-[0_0_15px_rgba(245,158,11,0.6)]"></div>
        ...
        <AnimatePresence>
          {selectedEvent && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6">
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 30 }} transition={{ type: "spring", damping: 25, stiffness: 220 }} className="relative w-full max-w-5xl bg-stone-900 text-stone-100 border border-stone-800 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(245,158,11,0.35)] flex flex-col md:flex-row max-h-[92vh] z-10" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="w-full md:w-[55%] p-6 md:p-8 flex flex-col max-h-[92vh] md:max-h-none overflow-y-auto custom-scrollbar">
                  ...
                  {/* Close button */}
                  <button onClick={() => setSelectedEvent(null)} className="absolute top-5 right-5 w-8 h-8 bg-stone-950 border border-stone-800 hover:border-amber-500 hover:text-amber-500 text-stone-400 rounded-full flex items-center justify-center transition-all z-20 font-mono text-sm">
                    ✕
                  </button>
                  ...
                  <div className="mb-6 pr-6">
                    ...
                    <h2 className="text-xl md:text-2xl font-bold text-stone-100 leading-tight mt-3 tracking-wide uppercase font-serif">
                      {isDecrypting ? (<span className="font-mono text-stone-400 text-lg tracking-widest uppercase">
                          Decrypting Memory Log...
                        </span>) : (<ScrambledText text={selectedEvent.title}/>)}
                    </h2>
                  </div>

                  {/* Decryption Screen Overlay if loading */}
                  <AnimatePresence mode="wait">
                    {isDecrypting ? (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col justify-center items-center py-16 font-mono text-stone-400">
                        <div className="relative w-48 h-1.5 bg-stone-950 border border-stone-850 rounded-full overflow-hidden mb-4">
                          <motion.div className="h-full bg-gradient-to-r from-amber-500 to-rose-500" style={{ width: `${decryptProgress}%` }}/>
                        </div>
                      ...
                      </motion.div>) : (<motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex-1 flex flex-col">
                        {/* Interactive Navigation HUD Tabs */}
                        <div className="flex border-b border-stone-800 gap-1 mb-6 font-mono text-xs">
                          <button onClick={() => setActiveTab('codex')} className={`px-4 py-2 border-b-2 transition-all uppercase tracking-wider ${activeTab === 'codex'
                    ? 'border-amber-500 text-amber-500 bg-amber-950/20'
                    : 'border-transparent text-stone-400 hover:text-stone-200'}`}>
                            Memory Codex
                          </button>
                          <button onClick={() => setActiveTab('tactical')} className={`px-4 py-2 border-b-2 transition-all uppercase tracking-wider ${activeTab === 'tactical'
                    ? 'border-amber-500 text-amber-500 bg-amber-950/20'
                    : 'border-transparent text-stone-400 hover:text-stone-200'}`}>
                            Milestones
                          </button>
                          <button onClick={() => setActiveTab('stream')} className={`px-4 py-2 border-b-2 transition-all uppercase tracking-wider ${activeTab === 'stream'
                    ? 'border-amber-500 text-amber-500 bg-amber-950/20'
                    : 'border-transparent text-stone-400 hover:text-stone-200'}`}>
                            Cognitive Sync
                          </button>
                        ...
                        <div className="flex-1 min-h-[220px]">
                          ...
                          {/* TAB 1: CODEX */}
                          {activeTab === 'codex' && (<div className="space-y-6">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                  ...
                                  <div className="bg-stone-950/50 border border-stone-800 p-3 rounded-lg flex flex-col items-center justify-center relative group cursor-pointer hover:border-amber-500/50 transition-all">
                                    ...
                                    <div className="relative w-14 h-14 flex items-center justify-center">
                                      ...
                                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                        ...
                                        <motion.path className="text-amber-500" strokeWidth="2.5" strokeDasharray={`${selectedEvent.stats.impact}, 100`} strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1, ease: "easeOut" }}/>
                                      </svg>
                                  ...
                                  <div className="bg-stone-950/50 border border-stone-800 p-3 rounded-lg flex flex-col items-center justify-center relative group cursor-pointer hover:border-amber-500/50 transition-all">
                                    ...
                                    <div className="relative w-14 h-14 flex items-center justify-center">
                                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                        <path className="text-stone-800" strokeWidth="2.5" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
                                        <motion.path className="text-orange-500" strokeWidth="2.5" strokeDasharray={`${selectedEvent.stats.legacy}, 100`} strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1, delay: 0.1, ease: "easeOut" }}/>
                                      </svg>
                                  ...
                                  <div className="bg-stone-950/50 border border-stone-800 p-3 rounded-lg flex flex-col items-center justify-center relative group cursor-pointer hover:border-amber-500/50 transition-all">
                                    ...
                                    <div className="relative w-14 h-14 flex items-center justify-center">
                                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                        <path className="text-stone-800" strokeWidth="2.5" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
                                        <motion.path className="text-rose-500" strokeWidth="2.5" strokeDasharray={`${selectedEvent.stats.reach}, 100`} strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1, delay: 0.2, ease: "easeOut" }}/>
                                      </svg>
                          ...
                          {activeTab === 'tactical' && (<div className="space-y-4">
                        ...
                        return (<div key={idx} onClick={() => setActiveMilestoneIndex(isActive ? null : idx)} className={`relative cursor-pointer transition-all duration-300 p-3 rounded-lg border ${isActive
                                ? 'bg-amber-950/20 border-amber-500/30'
                                : 'bg-stone-950/40 border-transparent hover:border-stone-800 hover:bg-stone-950/60'}`}>
                                      {/* Node Dot Indicator */}
                                      <div className={`absolute -left-[31px] top-4 w-4 h-4 rounded-full border-2 flex items-center justify-center z-10 transition-all ${isActive
                                ...
                                : 'border-stone-850 bg-stone-900'}`}>
                                      ...
                                      <AnimatePresence>
                                        {isActive && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mt-2 text-stone-300 text-xs leading-relaxed">
                          ...
                          {activeTab === 'stream' && (<div className="bg-stone-950/30 border border-stone-850 p-4 rounded-xl flex flex-col justify-between items-center text-center space-y-4">
                    ...
                    <div className="flex items-end justify-center gap-1 w-full px-6 h-12">
                                    {[...Array(12)].map((_, i) => (<div key={i} className="w-1.5 bg-amber-500 rounded-full origin-bottom animate-bounce-slow" style={{
                              ...
                              {/* Playback controls */}
                              <button onClick={handleToggleAudio} disabled={loadingAudio} className={`w-full py-3 px-4 font-mono text-xs uppercase tracking-widest font-bold rounded-lg border transition-all flex items-center justify-center gap-3 ${loadingAudio
                        ...
                        <div className="mt-8 pt-6 border-t border-stone-850 font-mono">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {selectedEvent.keyFigures.map((figure, idx) => (<div key={idx} className="flex items-center gap-2 bg-stone-950 border border-stone-850 px-3 py-1.5 rounded-md hover:border-amber-500/40 hover:bg-stone-900/60 transition-all cursor-help group relative">
                  ...
                  <div className="mt-auto pt-6 border-t border-stone-850 flex items-center justify-between text-[9px] text-stone-400 font-mono">
                    ...
                    <button onClick={() => setSelectedEvent(null)} className="text-amber-500/70 hover:text-amber-500 uppercase tracking-widest text-[9px] transition-colors">
