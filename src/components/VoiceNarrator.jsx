...
const VoiceNarrator = ({ text = "The Battle of Adwa in 1896 was a stunning victory for Ethiopia against Italian colonial forces. Led by Emperor Menelik II and Empress Taytu Betul, the Ethiopian forces secured their country's independence and became a symbol of anti-colonial resistance worldwide.", title = "The Story of Adwa" }) => {
    ...
    return (<div className="max-w-2xl mx-auto bg-stone-50 p-8 rounded-[2.5rem] border border-stone-200 shadow-2xl font-sans">
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-3 justify-center">
          <select value={language} onChange={(e) => setLanguage(e.target.value)} className="bg-stone-100 border border-stone-200 rounded-xl px-3 py-2 text-xs font-bold uppercase tracking-widest cursor-pointer">
            <option value="en">English</option>
            <option value="am">አማርኛ</option>
            <option value="om">Afaan Oromo</option>
          </select>

          <select value={style} onChange={(e) => setStyle(e.target.value)} className="bg-stone-100 border border-stone-200 rounded-xl px-3 py-2 text-xs font-bold uppercase tracking-widest cursor-pointer">
        ...
        <div className="bg-stone-100 p-6 rounded-2xl border border-stone-100">
          ...
          <div className="flex items-center gap-4">
            {/* Play/Pause Button */}
            <button onClick={handlePlay} disabled={loading} className="w-12 h-12 bg-stone-50 text-amber-600 rounded-full flex items-center justify-center shadow-lg hover:bg-stone-100 transition-colors disabled:opacity-50">
              {loading ? (<div className="w-5 h-5 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div>) : isPlaying ? (<svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5,0a1 1 0 012 0v4a1 1 0 11-2 0V8z" clipRule="evenodd"/></svg>) : (<svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168l4.2 3a1 1 0 010 1.664l-4.2 3A1 1 0 018 13V7a1 1 0 011.555-.832z" clipRule="evenodd"/></svg>)}
            </button>

            {/* Progress Bar */}
            <div className="flex-1 flex flex-col">
              <input type="range" min="0" max="100" value={progress} onChange={handleSeek} className="w-full h-1 bg-stone-100 rounded-full appearance-none cursor-pointer accent-amber-500"/>
        ...
        <div className="flex justify-between items-center text-xs font-bold text-stone-500 uppercase tracking-widest">
          ...
          <div className="flex gap-2">
            {[0.8, 1, 1.2, 1.5].map(s => (<button key={s} onClick={() => handleSpeedChange(s)} className={`px-2 py-1 rounded ${speed === s ? 'bg-amber-900/20 text-stone-900' : 'hover:text-stone-900'}`}>
                {s}x
              </button>))}
          </div>
        </div>
        ...
        {/* Download Button */}
        {audioUrl && (<div className="text-center pt-4 border-t border-stone-200">
            <a href={audioUrl} download={`${title.toLowerCase().replace(/\s+/g, '_')}.mp3`} className="text-xs font-bold uppercase text-amber-600 hover:text-amber-700 transition-colors flex items-center justify-center gap-2">
