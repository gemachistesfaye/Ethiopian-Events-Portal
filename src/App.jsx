...
const ProfileSettings = ({ user }) => {
    ...
    return (<div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans">
      <div className="mb-8">
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3">
           <div className="bg-stone-100 rounded-3xl border border-stone-200 shadow-sm p-4 sticky top-28 space-y-2">
             <button onClick={() => setActiveSettingsTab('profile')} className={`w-full text-left px-5 py-3.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all flex items-center gap-3 ${activeSettingsTab === 'profile' ? 'bg-stone-105 text-stone-900 shadow-md border border-stone-200/50' : 'text-stone-500 hover:bg-stone-100 hover:text-stone-900'}`}>
               <span className="text-lg">👤</span> Profile
             </button>
             <button onClick={() => setActiveSettingsTab('preferences')} className={`w-full text-left px-5 py-3.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all flex items-center gap-3 ${activeSettingsTab === 'preferences' ? 'bg-stone-105 text-stone-900 shadow-md border border-stone-200/50' : 'text-stone-500 hover:bg-stone-100 hover:text-stone-900'}`}>
               <span className="text-lg">⚙️</span> Preferences
             </button>
             <button onClick={() => setActiveSettingsTab('security')} className={`w-full text-left px-5 py-3.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all flex items-center gap-3 ${activeSettingsTab === 'security' ? 'bg-stone-105 text-stone-900 shadow-md border border-stone-200/50' : 'text-stone-500 hover:bg-stone-100 hover:text-stone-900'}`}>
               <span className="text-lg">🛡️</span> Security
             </button>
             
             <div className="pt-4 mt-4 border-t border-stone-200">
               <button onClick={() => supabase.auth.signOut()} className="w-full text-left px-5 py-3.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest text-rose-500 hover:bg-rose-950/30 transition-all flex items-center gap-3">
        ...
        <div className="lg:col-span-9 space-y-8">
          ...
          {activeSettingsTab === 'profile' && (<div className="bg-stone-100 rounded-[2.5rem] border border-stone-200 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
              {/* Profile Cover */}
              ...
              <div className="px-10 pb-10">
                 <div className="flex flex-col md:flex-row items-start md:items-end gap-6 -mt-16 mb-8 relative z-10">
                   <div className="w-32 h-32 bg-stone-100 rounded-3xl p-2 shadow-2xl">
                ...
                <form onSubmit={handleUpdateEmail} className="space-y-6 max-w-xl">
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder={user.email} className="flex-1 h-14 bg-stone-100 border border-stone-200 rounded-xl px-5 text-sm font-normal focus:border-amber-500 focus:outline-none focus:ring-4 focus:ring-amber-500/10 transition-all shadow-inner" required/>
                      <button type="submit" disabled={loading} className="h-14 px-6 bg-stone-105 text-stone-900 rounded-xl font-bold uppercase text-[11px] tracking-widest hover:bg-stone-200 transition-colors disabled:opacity-50 whitespace-nowrap shadow-lg border border-stone-200/50">
                        {loading ? 'Saving...' : 'Update Email'}
                      </button>
                    </div>
                  </div>
                  {message && <p className="text-xs font-bold text-amber-600 bg-amber-900/20 p-3 rounded-lg border border-amber-100">{message}</p>}
                </form>
              </div>
            </div>)}

          {activeSettingsTab === 'preferences' && (<div className="bg-stone-100 rounded-[2.5rem] border border-stone-200 shadow-xl p-10 animate-in fade-in zoom-in-95 duration-300">
              ...
              <div className="space-y-8 font-normal text-xs md:text-sm">
                <div className="flex items-center justify-between border-b border-stone-200 pb-8">
                   ...
                   <div className="w-14 h-8 bg-amber-900/20 rounded-full relative cursor-pointer shadow-inner">
                     <div className="w-6 h-6 bg-stone-100 rounded-full absolute right-1 top-1 shadow-sm"></div>
                   </div>
                </div>
                ...
                <div className="flex items-center justify-between border-b border-stone-200 pb-8">
                   ...
                   <div className="w-14 h-8 bg-stone-700 rounded-full relative cursor-pointer shadow-inner">
                     <div className="w-6 h-6 bg-stone-100 rounded-full absolute left-1 top-1 shadow-sm"></div>
                   </div>
                </div>
          ...
          {activeSettingsTab === 'security' && (<div className="space-y-8 animate-in fade-in zoom-in-95 duration-300 font-sans">
...
const Dashboard = ({ user, onNavigate }) => {
    ...
    return (<div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-stone-100 rounded-[3rem] p-10 md:p-16 text-stone-900 shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
        ...
        <div className="w-32 h-32 bg-stone-100 border-4 border-amber-500 rounded-full flex items-center justify-center text-5xl flex-shrink-0 relative z-10 shadow-2xl">
          👋
        </div>
      ...
      <div className="grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <button onClick={() => onNavigate('vault')} className="bg-stone-100 p-8 rounded-[2.5rem] border border-stone-200 shadow-xl hover:border-amber-400 hover:shadow-2xl transition-all group text-left h-full flex flex-col relative overflow-hidden">
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-amber-900/20 rounded-[2.5rem] p-8 shadow-xl text-stone-900 relative overflow-hidden">
...
const UpdatePassword = ({ onComplete }) => {
    ...
    return (<div className="text-center py-10 bg-stone-105 p-10 rounded-[2.5rem] border border-stone-200 shadow-xl max-w-md mx-auto mt-10 font-sans">
      ...
      <form onSubmit={handleUpdatePassword} className="space-y-4">
        <div>
          ...
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full h-12 bg-stone-100 border border-stone-200 rounded-xl px-4 text-sm font-normal focus:border-amber-500 focus:outline-none focus:bg-stone-100 transition-all" required/>
        </div>
        <div>
          <label className="text-[10px] font-bold text-stone-500 uppercase tracking-widest mb-1 block text-left">Confirm Password</label>
          <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full h-12 bg-stone-100 border border-stone-200 rounded-xl px-4 text-sm font-normal focus:border-amber-500 focus:outline-none focus:bg-stone-100 transition-all" required/>
        </div>
...
const App = () => {
    ...
    const [activeTab, setActiveTab] = useState(() => {
        return localStorage.getItem('ethio_active_tab') || 'home';
    });
    useEffect(() => {
    ...
    const handleAddReminder = (dateStr, note, priority, category, eventId) => {
        const newReminder = {
    ...
    const handleUpdateReminder = (id, note) => {
    ...
    return (<div className="min-h-screen bg-stone-100 selection:bg-amber-200 flex flex-col pb-20 md:pb-0 font-sans">
      {/* Dynamic Header */}
        ...
        <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex gap-0.5 bg-stone-50/40 p-1 rounded-2xl border border-stone-200/80 shadow-inner">
            ...
            ...(user ? [
                ...
                {tab.id === 'reminders' && reminders.length > 0 && (<span className="bg-amber-900/20 text-stone-950 w-5 h-5 flex items-center justify-center rounded-lg shadow-sm font-bold ml-1">{reminders.length}</span>)}
              </button>))}

          </nav>

          <div className="flex items-center gap-4">
            <select onChange={(e) => i18n.changeLanguage(e.target.value)} value={i18n.language} className="bg-stone-50 border border-stone-200/80 rounded-xl px-3 py-2 text-xs font-black cursor-pointer focus:outline-none focus:ring-2 focus:ring-amber-500/20">
              <option value="en">EN</option>
              <option value="am">አማ</option>
              <option value="om">ORM</option>
            </select>
            <button onClick={() => setActiveTab('account')} className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all ${activeTab === 'account' ? 'bg-amber-600 border-amber-700 text-stone-950 shadow-lg' : 'bg-stone-50 border-stone-200/80 text-stone-500 hover:bg-stone-900 hover:text-stone-50 hover:border-stone-950'}`} title="Profile">
      ...
      <main className="max-w-7xl mx-auto px-8 py-10 flex-grow w-full">
        {isResettingPassword ? (<UpdatePassword onComplete={() => setIsResettingPassword(false)}/>) : !user ? (activeTab === 'account' ? (<Auth />) : (<LandingPage onExplore={() => setActiveTab('account')}/>)) : activeTab === 'vault' ? (<div className="grid lg:grid-cols-12 gap-12">
            <div className="lg:col-span-8 space-y-12">
              <section className="animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-10">
                   <div>
                     <span className="bg-stone-100 text-amber-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] mb-3 inline-block shadow-lg">The Explorer</span>
                   ...
                   <div className="bg-stone-100 p-6 rounded-3xl border border-stone-200 shadow-xl flex items-center gap-6">
                     <div className="w-12 h-12 bg-amber-900/20 rounded-2xl flex items-center justify-center font-black text-2xl text-stone-900 shadow-xl">{todayInfo.eth.day}</div>
                      <div>
                        <p className="text-[10px] font-black text-stone-500 uppercase tracking-widest leading-none mb-1">Ethiopian Date</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                  <div className="md:col-span-2 relative">
                    <input type="text" placeholder="Search events, history, or locations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-16 bg-stone-100 border-2 border-stone-200 rounded-[1.5rem] px-14 text-sm font-bold focus:border-amber-500 focus:outline-none focus:ring-4 focus:ring-amber-500/10 transition-all shadow-sm"/>
                    <svg className="w-6 h-6 absolute left-6 top-5 text-stone-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                  </div>
                  <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="h-16 bg-stone-100 border-2 border-stone-200 rounded-[1.5rem] px-6 text-xs font-black uppercase tracking-widest focus:border-amber-500 focus:outline-none shadow-sm cursor-pointer">
                ...
                <Calendar onSelectDate={(day) => setSelectedDay(day)} selectedDate={selectedDay?.gregorian || null}/>
              </section>
            </div>

            <div className="lg:col-span-4">
              <div className="sticky top-28 space-y-8">
                <div className="bg-stone-100 p-10 rounded-[3rem] border border-stone-200 shadow-2xl relative overflow-hidden group">
                  <div className="absolute top-0 left-0 w-2 h-full bg-stone-100 group-hover:bg-amber-900/20 transition-colors"></div>
                  
                  {selectedDay ? (<div className="animate-in fade-in slide-in-from-right-4 duration-500">
                      <div className="mb-10">
                         ...
                         <p className="text-stone-500 text-xs font-black uppercase tracking-widest ethiopic-font">
                           {selectedDay.ethiopian.monthNameAmharic} {selectedDay.ethiopian.day}
                         </p>
                      </div>
                      
                      <div className="space-y-6">
                        {selectedDay.events.length > 0 ? (selectedDay.events.map((event) => (<div key={event.id} className="group bg-stone-100 p-6 rounded-[2rem] border border-stone-200 hover:border-amber-400 hover:bg-stone-100 transition-all cursor-pointer shadow-sm hover:shadow-xl" onClick={() => setSelectedEvent(event)}>
                              <div className="relative h-44 rounded-[1.5rem] overflow-hidden mb-5">
                                ...
                                <span className="absolute bottom-4 left-4 bg-amber-900/20 text-stone-900 text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-widest">{event.category}</span>
                              </div>
                              <h4 className="font-black text-stone-900 text-xl group-hover:text-amber-600 transition-colors mb-1">{event.name}</h4>
                              <p className="text-stone-500 text-[10px] font-bold uppercase tracking-widest">Click for Deep Insight</p>
                            </div>))) : (<div className="py-24 text-center bg-stone-100/50 rounded-[2.5rem] border-2 border-dashed border-stone-200">
                             <div className="w-16 h-16 bg-stone-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-stone-200">
                               <svg className="w-8 h-8 text-stone-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                             </div>
                             <p className="text-stone-700 font-black text-[10px] uppercase tracking-widest">No Heritage Records</p>
                          </div>)}
                        
                        <div className="pt-8 border-t border-stone-200 flex items-center justify-center">
                           <button onClick={() => setActiveTab('vault')} className="text-stone-500 hover:text-amber-600 text-[10px] font-black uppercase tracking-widest transition-colors">
                             Manage Personal Heritage Box →
                           </button>
                        </div>
                      </div>
                    </div>) : (<div className="py-32 text-center">
                      <div className="w-24 h-24 bg-stone-100 rounded-[2rem] flex items-center justify-center mx-auto mb-8 border border-stone-200 rotate-6 transition-transform shadow-sm">
                        <svg className="w-12 h-12 text-stone-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                      </div>
                      <p className="text-stone-700 font-black text-[10px] uppercase tracking-widest max-w-[140px] mx-auto leading-relaxed">Select a portal date to explore history</p>
                    </div>)}
                </div>

                <div className="bg-stone-100 rounded-[3rem] p-10 text-stone-900 shadow-2xl relative overflow-hidden group border border-stone-200">
                  ...
                  <h4 className="text-lg font-black text-amber-600 mb-4 tracking-tight flex items-center gap-3">
                     💡 Quick Cultural Fact
                  </h4>
            ...
            <div className="lg:col-span-12 mt-12 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-100">
              <MyReminders reminders={reminders} events={EVENTS_DATA} onDelete={handleDeleteReminder} onUpdate={handleUpdateReminder} onClearAll={handleClearAllReminders}/>
            </div>
          </div>) : activeTab === 'home' ? (user ? <Dashboard user={user} onNavigate={setActiveTab}/> : <LandingPage onExplore={() => setActiveTab('account')}/>) : activeTab === 'map' ? (<CulturalMap />) : activeTab === 'timeline' ? (<HistoricalTimeline />) : activeTab === 'culture' ? (<CultureZone />) : activeTab === 'chat' ? (<HeritageChat />) : activeTab === 'account' ? (user ? <ProfileSettings user={user}/> : <Auth />) : (user ? (<ProfileSettings user={user}/>) : (<Auth />))}
      </main>
      <Footer />
      {selectedEvent && (<EventDetails event={selectedEvent} onClose={() => setSelectedEvent(null)} onAddReminder={(eventId, note, priority, category) => {
...
const Footer = () => {
    const socialLinks = [
        ...
        { id: 'gh', icon: 'M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12', color: 'bg-stone-100 hover:shadow-stone-200', href: 'https://github.com/gemachistesfaye' },
        { id: 'mail', icon: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', color: 'bg-rose-950/300 hover:shadow-rose-200', href: 'mailto:gemachistesfaye36@gmail.com', isStroke: true },
    ...
    return (<footer className="bg-stone-100 border-t border-stone-200 mt-12 py-6 px-6">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-6 items-center">
        
        {/* Left Column: About Website */}
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <span className="bg-stone-100 text-amber-600 px-2.5 py-0.5 rounded text-[9px] font-black uppercase tracking-widest">
              Portal
            </span>
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-stone-700">© 2026 Rights Reserved</p>
        </div>

        {/* Right Column: About Me */}
        <div className="flex flex-col md:items-end gap-2 text-center md:text-right">
          <div className="flex items-center gap-3 md:flex-row-reverse">
            <div className="w-8 h-8 bg-stone-100 rounded-lg flex items-center justify-center text-amber-600 font-black text-sm shadow-md">G</div>
            <div>
          ...
          <div className="flex gap-1.5 mt-1 justify-center md:justify-end">
            {socialLinks.map((link) => (<a key={link.id} href={link.href} target="_blank" rel="noopener noreferrer" className={`w-7 h-7 ${link.color} text-stone-900 rounded-md flex items-center justify-center transition-all hover:-translate-y-0.5 shadow-sm group`}>
                <svg className="w-3.5 h-3.5 group-hover:scale-110 transition-transform" fill={link.isStroke ? "none" : "currentColor"} stroke={link.isStroke ? "currentColor" : "none"} strokeWidth={link.isStroke ? 2.5 : 0} viewBox="0 0 24 24">
