...
const ProfileSettings: React.FC<{ user: any }> = ({ user }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8">
         <h2 className="text-4xl font-black text-stone-100 tracking-tight">Account Settings</h2>
         <p className="text-stone-400 font-medium mt-2">Manage your portal identity, preferences, and security settings.</p>
      </div>
                 ...
                 <div className="flex flex-col md:flex-row items-start md:items-end gap-6 -mt-16 mb-8 relative z-10">
                   ...
                   <div className="flex-1">
                     <div className="flex items-center gap-3 mb-1">
                       <h3 className="text-3xl font-black text-stone-100">{user.email?.split('@')[0]}</h3>
                       <span className="bg-amber-900/200 text-stone-100 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">Verified</span>
                     </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <button 
                        type="submit" 
                        disabled={loading}
                        className="h-14 px-8 bg-stone-950 text-white rounded-xl font-black uppercase text-xs tracking-widest hover:bg-stone-800 transition-colors disabled:opacity-50 whitespace-nowrap shadow-lg hover:shadow-stone-900/20"
                      >
...
const Dashboard: React.FC<{ user: any, onNavigate: (tab: string) => void }> = ({ user, onNavigate }) => {
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto">
      {/* Welcome Header */}
      <div className="bg-stone-950 rounded-[3rem] p-10 md:p-16 text-white shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
        ...
        <div className="relative z-10 text-center md:text-left flex-1">
          <span className="bg-amber-900/200/20 text-amber-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] mb-4 inline-block">Pro Explorer</span>
          ...
          <p className="text-sm text-stone-400 max-w-md">Your gateway to Ethiopian heritage is ready. Discover deep historical insights and upcoming events.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="flex items-center justify-between">
             <h3 className="text-2xl font-black text-stone-100 tracking-tight">Explore Platform</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <button onClick={() => onNavigate('vault')} className="bg-stone-950 p-8 rounded-[2.5rem] border border-stone-800 shadow-xl hover:border-amber-400 hover:shadow-2xl transition-all group text-left h-full flex flex-col relative overflow-hidden">
              ...
              <h3 className="text-2xl font-black text-stone-100 mb-3 relative z-10">Heritage Calendar</h3>
              <p className="text-sm text-stone-400 leading-relaxed font-medium relative z-10 flex-1">Explore upcoming events, festivals, and religious holidays synchronized across Gregorian and Ethiopian dates.</p>
            ...
            <button onClick={() => onNavigate('chat')} className="bg-stone-950 p-8 rounded-[2.5rem] border border-stone-800 shadow-xl hover:border-emerald-400 hover:shadow-2xl transition-all group text-left h-full flex flex-col relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:scale-150 transition-transform duration-700">
            ...
            <button onClick={() => onNavigate('map')} className="bg-stone-950 p-8 rounded-[2.5rem] border border-stone-800 shadow-xl hover:border-blue-400 hover:shadow-2xl transition-all group text-left h-full flex flex-col relative overflow-hidden">
              ...
              <h3 className="text-xl font-black text-stone-100 mb-2 relative z-10">Cultural Atlas</h3>
            ...
            <button onClick={() => onNavigate('culture')} className="bg-stone-950 p-8 rounded-[2.5rem] border border-stone-800 shadow-xl hover:border-rose-400 hover:shadow-2xl transition-all group text-left h-full flex flex-col relative overflow-hidden">
              ...
              <h3 className="text-xl font-black text-stone-100 mb-2 relative z-10">Culture Zone</h3>
              <p className="text-xs text-stone-400 leading-relaxed font-medium relative z-10 flex-1">Food, music, and traditions.</p>
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-amber-900/200 rounded-[2.5rem] p-8 shadow-xl text-stone-100 relative overflow-hidden">
              ...
              <button onClick={() => onNavigate('vault')} className="bg-stone-950 text-white px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-xl hover:bg-stone-800 transition-colors relative z-10">
                 View Details
              </button>
           </div>
           ...
           <div className="bg-stone-950 rounded-[2.5rem] border border-stone-800 shadow-xl p-8">
              ...
              <div className="space-y-6">
                 <div className="flex gap-4 items-start">
                    <div className="w-10 h-10 rounded-xl bg-stone-800 flex items-center justify-center text-lg flex-shrink-0">💬</div>
                    <div>
                       <p className="text-sm font-bold text-stone-100">Asked about Axumite Kingdom</p>
                 ...
                 <div className="flex gap-4 items-start">
                    <div className="w-10 h-10 rounded-xl bg-stone-800 flex items-center justify-center text-lg flex-shrink-0">🔖</div>
                    <div>
                       <p className="text-sm font-bold text-stone-100">Saved Timkat Event</p>
                       <p className="text-[10px] font-black uppercase tracking-widest text-stone-400 mt-1">1 week ago</p>
                    </div>
                 </div>
                 <div className="flex gap-4 items-start">
                    <div className="w-10 h-10 rounded-xl bg-stone-800 flex items-center justify-center text-lg flex-shrink-0">🗺️</div>
                    <div>
                       <p className="text-sm font-bold text-stone-100">Explored Lalibela Map</p>
    ...
    <div className="text-center py-10 bg-stone-950 p-10 rounded-[2.5rem] border border-stone-800 shadow-xl max-w-md mx-auto mt-10">
      ...
      <form onSubmit={handleUpdatePassword} className="space-y-4">
        ...
        <button 
          type="submit" 
          disabled={loading}
          className="w-full h-12 bg-amber-900/200 text-stone-100 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-amber-600 transition-colors disabled:opacity-50"
        >
      ...
      <header className="bg-stone-950/80 backdrop-blur-xl border-b border-stone-800 sticky top-0 z-40 shadow-sm transition-all">
        <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black tracking-tighter text-stone-100 leading-none">{t('app.title')}</h1>
              <p className="text-[9px] text-stone-400 font-black uppercase tracking-[0.3em] mt-1">{t('app.subtitle')}</p>
            </div>
          </div>
          
          <nav className="hidden lg:flex gap-0.5 bg-stone-800 p-1 rounded-2xl border border-stone-800 shadow-inner">
            {[
              ...
              >
                ...
                {tab.id === 'reminders' && reminders.length > 0 && (
                   <span className="bg-amber-900/200 text-stone-100 w-5 h-5 flex items-center justify-center rounded-lg shadow-sm font-bold ml-1">{reminders.length}</span>
                )}
              </button>
            ))}
          ...
          <div className="flex items-center gap-4">
            <select 
              ...
              value={i18n.language}
              className="bg-stone-800 border border-stone-800 rounded-xl px-3 py-2 text-xs font-black cursor-pointer focus:outline-none focus:ring-2 focus:ring-amber-500/20"
            >
            ...
            <button 
               ...
               className={`w-10 h-10 rounded-full border flex items-center justify-center transition-colors ${activeTab === 'account' ? 'bg-amber-600 border-amber-700 text-white shadow-lg' : 'bg-stone-800 border-stone-800 text-stone-400 hover:bg-stone-700 hover:text-stone-200'}`}
               title="Profile"
            >
          ...
          <div className="grid lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-28 space-y-8">
                <div className="bg-stone-950 p-10 rounded-[3rem] border border-stone-800 shadow-2xl relative overflow-hidden group">
                  ...
                  {selectedDay ? (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                      <div className="mb-10">
                         <h3 className="text-3xl font-black text-stone-100 leading-none mb-2">{selectedDay.gregorian.getDate()} {new Intl.DateTimeFormat('en-US', { month: 'long' }).format(selectedDay.gregorian)}</h3>
                      ...
                      <div className="space-y-6">
                          ...
                          selectedDay.events.map((event: EthiopianEvent) => (
                            ...
                            >
                              <div className="relative h-44 rounded-[1.5rem] overflow-hidden mb-5">
                                ...
                                <span className="absolute bottom-4 left-4 bg-amber-900/200 text-stone-100 text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-widest">{event.category}</span>
                              </div>
                              <h4 className="font-black text-stone-100 text-xl group-hover:text-amber-600 transition-colors mb-1">{event.name}</h4>
                        ...
                        <div className="pt-8 border-t border-stone-800 flex items-center justify-center">
                           <button 
                ...
                <div className="bg-stone-950 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group border border-stone-800">
      ...
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-6 items-center">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <span className="bg-stone-950 text-amber-500 px-2.5 py-0.5 rounded text-[9px] font-black uppercase tracking-widest">
              Portal
            </span>
            <h3 className="text-lg font-black text-stone-100 tracking-tight">
              Ethiopian Events <span className="text-amber-600">Portal</span>
            </h3>
          </div>
          <p className="text-stone-400 text-xs font-medium max-w-sm">
        ...
        <div className="flex flex-col md:items-end gap-2 text-center md:text-right">
          <div className="flex items-center gap-3 md:flex-row-reverse">
            <div className="w-8 h-8 bg-stone-950 rounded-lg flex items-center justify-center text-amber-500 font-black text-sm shadow-md">G</div>
            <div>
              <h4 className="text-sm font-black text-stone-100">Gemachis Tesfaye</h4>
              <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest">Lead Engineer</p>
            </div>
          </div>
          <p className="text-stone-400 text-xs font-medium max-w-sm md:text-right">
